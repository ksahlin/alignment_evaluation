...
# chr_id="GCF_003018135.1_ASM301813v1_genomic"
...
if [[ ! -f $refs/$chr_id.vcf ]]; then
    ...
    mason_variator --sv-indel-rate 0.00005 --snp-rate 0.005 --small-indel-rate 0.0005 --max-small-indel-size 50   -ir $refs/$chr_id.fa -ov $refs/$chr_id.vcf &> /dev/null
fi
...
for read_length in 200 250 300 #100 150 
do 
    for chr_id in sim_repeat_genome500 # GCF_003018135.1_ASM301813v1_genomic # sim_repeat_genome5 #hg38_chr21 hg38_chr18 hg38_chrX hg38_chr1 hg38_chr15 # sim_repeat_genome500 # sim_50contigs # hg38_chr1_2 hg38_chr6_15_18_X_Y  #  # hg38_chr1_2 hg38_chr6_15_18_X_Y  #
    do
        ...
        then    
          ...
          if  ((read_length >= 250));
            then  
                echo mason_simulator -ir $refs/$chr_id.fa -n 500000 -iv $refs/$chr_id.vcf --illumina-read-length $read_length --fragment-mean-size 700 -o $outroot/$chr_id/$read_length.L.fq -or $outroot/$chr_id/$read_length.R.fq -oa $outroot/$chr_id/$read_length.sam
                mason_simulator -ir $refs/$chr_id.fa -n 500000 -iv $refs/$chr_id.vcf --illumina-read-length $read_length --fragment-mean-size 700 -o $outroot/$chr_id/$read_length.L.fq -or $outroot/$chr_id/$read_length.R.fq -oa $outroot/$chr_id/$read_length.sam
            else
                echo mason_simulator -ir $refs/$chr_id.fa -n 500000 -iv $refs/$chr_id.vcf --illumina-read-length $read_length --fragment-mean-size 300 -o $outroot/$chr_id/$read_length.L.fq -or $outroot/$chr_id/$read_length.R.fq -oa $outroot/$chr_id/$read_length.sam
                mason_simulator -ir $refs/$chr_id.fa -n 500000 -iv $refs/$chr_id.vcf --illumina-read-length $read_length --fragment-mean-size 300 -o $outroot/$chr_id/$read_length.L.fq -or $outroot/$chr_id/$read_length.R.fq -oa $outroot/$chr_id/$read_length.sam
          fi
        fi


        ###### run the tools ######
        # echo  $strobealign_dev_dir/./strobealign -t 1 -r $read_length $refs/$chr_id.fa $outroot/$chr_id/$read_length.L.fq $outroot/$chr_id/$read_length.R.fq 
        ...
        do
            ...
            # get map location stats
            echo -n $chr_id,$read_length,$tool,align,
            python $eval_script_dir/get_accuracy.py --truth $outroot/$chr_id/$read_length.sam --predicted_sam $outroot/$chr_id/$read_length.$tool.bam --time_mem $outroot/$chr_id/$read_length.$tool.stderr
            ...
            # bcftools call --threads 3 -v -c -O v $outroot/$chr_id/$read_length.$tool.vcf.gz > $outroot/$chr_id/$read_length.$tool.variants.vcf 2> /dev/null
